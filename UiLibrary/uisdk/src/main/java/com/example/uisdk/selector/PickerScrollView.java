...
public class PickerScrollView extends View {
    ...
    private static List<String> fDataList = new ArrayList<>();
    /**
    ...
    @SuppressLint("NewApi")
    private int mColorText = getContext().getColor(R.color.colorWhite);
     ...
     * 默认允许滑动
     */
    private static boolean isNoScroll = true;
    ...
    private void performSelect(int position) {
        if (mSelectListener != null)
            mSelectListener.onSelect(mDataList.get(position));
    }
    ...
    private void moveHeadToTail() {
        if (mDataList.size() > 1 && mDataList.size() <= listMax) {
            String datasBean = mDataList.get(0);
            mDataList.remove(0);
            fDataList.add(datasBean);
            isNoScroll = true;
        } else {
            isNoScroll = false;
        }
    ...
    public void setDetermine() {
        ...
        if (onChooseListener != null) {
            // 因为第一个就是需要选中的日期
            onChooseListener.onChoose(mDataList.get(0));
    ...
    private void moveTailToHead() {
        if (fDataList.size() > 0) {
            String datasBean = fDataList.get(fDataList.size() - 1);
            fDataList.remove(fDataList.size() - 1);
    ...
    public interface OnChooseListener {
        void onChoose(String chooseDate);
    }
